...
namespace mars
{
    namespace vsg_graphics
    {
        ...
        {
            auto physicalDevice = GuiHelper::device->getPhysicalDevice();
            ...
            const VkExtent2D& extent)
        {
            ...
            auto targetFormat = supportsBlit(sourceFormat) ? VK_FORMAT_R8G8B8A8_SRGB : sourceFormat;
            ...
            image->compile(GuiHelper::device);

            auto memReqs = image->getMemoryRequirements(GuiHelper::device->deviceID);
            auto memFlags = VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT;
            auto deviceMemory = vsg::DeviceMemory::create(GuiHelper::device, memReqs, memFlags);
            ...
            VkSampleCountFlagBits samples)
        {
            ...
            return vsg::createImageView(GuiHelper::device, image, vsg::computeAspectFlagsForFormat(format));
        }
            ...
            bool requiresDepthRead)
        {
            ...
            {
                return createTransferRenderPass(imageFormat, depthFormat, requiresDepthRead);
            }
            ...
            VkSampleCountFlagBits const samples)
        {
            ...
            {
                // MSAA
                imageViews.emplace_back(createTransferImageView(imageFormat, extent, samples));
                imageViews.emplace_back(transferImageView);
                imageViews.emplace_back(createTransferImageView(depthFormat, extent, samples));
                if (requiresDepthRead)
                {
                    imageViews.emplace_back(createTransferImageView(depthFormat, extent, VK_SAMPLE_COUNT_1_BIT));
                }
            }

            auto renderPass = createTransferRenderPass(imageFormat, depthFormat, samples, requiresDepthRead);
        ...
        vsg::ref_ptr<vsg::Data> getImageData(vsg::ref_ptr<vsg::Viewer> viewer, vsg::ref_ptr<vsg::Image> captureImage)
        {
            ...
            vkGetImageSubresourceLayout(*(GuiHelper::device), captureImage->vk(GuiHelper::device->deviceID), &subResource, &subResourceLayout);

            auto deviceMemory = captureImage->getDeviceMemory(GuiHelper::device->deviceID);
